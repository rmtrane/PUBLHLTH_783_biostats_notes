...
For a continuous distribution, can we specify the probability of every single possible outcome?
--
...
```{r}
X <- Normal(mu = 4, sigma = 1)

X_samples <- tibble(x = random(X, n = 1000000)) %>% mutate(i = row_number())
```


Observe $10$ realizations of a random variable $X$, draw a histogram with $10$ bins.
...
normal_histogram <- function(I = 10, bins = 10){
  ggplot(data = X_samples %>% filter(i <= I),
    ...
    geom_histogram(bins = bins, boundary = 0,
                   aes(y = ..density..)) + 
    scale_x_continuous(limits = c(floor(min(X_samples$x)), ceiling(max(X_samples$x))),
                       breaks = floor(min(X_samples$x)):ceiling(max(X_samples$x)))
}
...
Observe $100$ realizations, draw a histogram with $20$ bins.
...
Observe $1000$ realizations, draw a histogram with $75$ bins.
...
Observe $10000$ realizations, draw a histogram with $100$ bins
...
Observe $10000$ realizations, draw a histogram with $125$ bins.
...
Let's try with $1000000$ realizations, and $150$ bins. 
