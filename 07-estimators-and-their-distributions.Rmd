...
The average depression score in this sample is `r dep_score_averages %>% filter(i == 1) %>% pull(average)`. Rinse and repeat `r format(N, scientific = FALSE)` times. 
...
if(!file.exists('data/R_output/RRs.Rds')){
  RRs <- samples_RR %>% 
    ...
    nest() %>%
    ungroup() %>% 
    mutate(RR = map_dbl(data, ~RR(.x %>% mutate(gender = factor(gender), 
                                                depression_severity_binary = factor(depression_severity_binary)), 
                                  gender, depression_severity_binary)))

  readr::write_rds(RRs, path = 'data/R_output/RRs.Rds')
} else {
  RRs <- readr::read_rds(path = 'data/R_output/RRs.Rds')
}
...
if(!file.exists('data/R_output/ORs.Rds')){
  ...
  readr::write_rds(ORs, path = 'data/R_output/ORs.Rds')
} else {
  ORs <- readr::read_rds(path = 'data/R_output/ORs.Rds')
}
...
ORs %>% 
  ggplot(aes(x = OR)) + 
    geom_histogram(aes(y = ..density..),
                   bins = floor(N/300)) + 
    geom_vline(aes(xintercept = true_OR, color = 'True OR'), linetype = 'dashed') + 
    scale_y_continuous(expand = expand_scale(mult = c(0, 0.025))) + 
    labs(color = '')
```
...
logOR_var <- show_depr_by_gender %>% 
  ...
  mutate(group_total = sum(n)) %>% 
  ungroup() %>% 
  mutate(p_gender = group_total/sum(n)) %>%       ## group proportions
  mutate(var_contribution = 1/(p_gender*sample_size_RR*n/group_total)) %>% ## 1 over expected n in group times expected number 
  pull(var_contribution) %>% sum
# 
# p_male <- mean(filter(show_depr_by_gender, gender == 1)$depression_severity_binary)
# p_female <- mean(filter(show_depr_by_gender, gender == 0)$depression_severity_binary)
# 
# logOR_var <- sum(1/(c(p_male*sample_size_RR, (1-p_male)*sample_size_RR,
#                      p_female*sample_size_RR, (1-p_female)*sample_size_RR)))
...
From the SHOW population, we find that the mean height is $\mu = `r round(mean(show_heights[["height"]]), digits = 2)`$, and the variance is $\sigma^2 = `r round(var(show_heights[["height"]]), digits = 2)`$. So, since the sample size here was $n = `r sample_size`$, the CLT tells us that $\bar{X} \sim N(`r round(true_mean, digits = 2)`, `r round(true_var, digits = 2)`/`r sample_size`)$. How does this fit the histogram? The black line below is that exact normal distribution. Fits pretty well, if you ask me.
...
To explore more, you can spend a few minutes [here](https://rtrane.shinyapps.io/CLT_2/). Data can be simulated from a number of different distribution, or you can upload a data set (such as the FHS data set).
