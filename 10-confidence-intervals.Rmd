...
tibble(x = seq(-4, 4, by = 0.01),
  ...
  ggplot(aes(x = x, y = y)) + 
    ...
    scale_x_continuous(breaks = c(seq(-4, 4, by = 2), -1.5, 1.5),
                       labels = c(seq(-4, 4, by = 2), parse(text = bquote(-z[obs])), bquote(z[obs]))) +
...
tibble(x = seq(-4, 4, by = 0.01),
  ...
  ggplot(aes(x = x, y = y)) + 
    ...
    scale_x_continuous(breaks = c(seq(-3, 3, by = 2), 0, -1.96, 1.96),
                       labels = c(seq(-3, 3, by = 2), 0, parse(text = bquote(-z[crit1])), bquote(z[crit2]))) +
...
\begin{align*}
  s_p^2 &= \frac{1628 \cdot 49.92 + 645 \cdot 89.29}{1629 + 646 - 2} \\
        ...
        &= `r (s_p2 <- round((1628 * 49.92 + 645 * 89.29)/(1629 + 646 - 2), digits = 2))`
\end{align*}
...
\begin{align*}
  (\bar{X}_1 - \bar{X}_0) \pm 1.96 \cdot s_p \sqrt{1/n_1 + 1/n_0} &= `r (31.16 - 29.23)` \pm 1.96 \cdot `r round(s_p, digits = 2)` \cdot `r round(sqrt(1/646 + 1/1629), digits = 2)` \\
  &= `r (31.16 - 29.23)` \pm `r round(1.96*s_p*sqrt(1/646 + 1/1629), digits = 2)` \\
  &= [`r paste(diff_in_means, collapse = ", ")`]
\end{align*}

We're $95\%$ confident that the true difference in means lies in the interval [`r paste(diff_in_means, collapse = ", ")`]. This also means that, if we were to test if there is a difference at a $5\%$ significance level, we would reject the null hypothesis of $H_0: \mu_1 - \mu_0 = 0$, since $0$ is not in the interval. Remember, the interval consists of all the values we would NOT reject, so all values outside the interval would be rejected. 
...
```{r}
diff_in_props <- round(0.4084357 - 0.3937381 + c(-1,1)*1.96*sqrt(0.4031431*(1-0.4031431)*(1/765 + 1/415)), digits = 3)
...
\begin{align*}
  ...
  &= (0.408 - 0.394) \pm 1.96 \cdot \sqrt{0.403\cdot (1 - 0.403\cdot)(1/765 + 1/415)} \\
  &=  `r 0.408 - 0.394` \pm `r 1.96 * sqrt(0.403*(1-0.403)*(1/765 + 1/415))`\\
