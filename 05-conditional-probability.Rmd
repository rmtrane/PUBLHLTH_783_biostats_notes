...
To find the conditional probability, you basically narrow down the universe you operate in. Instead of asking "how many individuals have mild depression out of all individuals?" you ask "how many individuals have mild depression out of **individuals that are divorced**?". So, in other words, all you worry about is the second row in the table. We estimate the conditional probability of having a mild depression given the subject is divorced as $P(\text{mild depression} | \text{divorced}) = \frac{76}{416} \approx `r round(76/416, digits = 3)`$. 

```{r}
dep_sev_by_marital %>% 
  pander::pander(split.table = Inf, emphasize.strong.rows = 2)
```
...
Since $P(A \text{ and } B) = P(B \text{ and } A)$, equation \@ref(eq:bayes1) gives us that $P(B | A)P(A) = P(A \text{ and } B)$, and so equation \@ref(eq:bayes2) follows by plugging this in to the numerator in equation \@ref(eq:bayes1). 
...
$$
  P(\text{patient has the disease} | \text{screen positive}) = \frac{P(\text{screen positive} | \text{patient has the disease}) \cdot P(\text{patient has the disease})}{P(\text{screen positive})}.
$$
...
We will consider where the events $A = \text{subject is divorced}$ and $B = \text{subject is mildly depressed}$ are independent. For simplicity, we only consider subjects for which we know both marital status and depression severity, i.e. any subjects with missing data in one of the two variables have been removed.
...
While definition \@ref(def:ind1) is very useful when doing math, it might not make a whole lot of sense intuitively. Definition \@ref(def:ind2), on the other hand, I would argue aligns with our intuitive understanding of independence. 

Intuitively, two events, $A$ and $B$, are independent if $A$ provides no information of $B$, and vice versa. Now, if $A$ provides no information about $B$, then what would the difference between $P(B)$ and $P(B|A)$ be? Remember, the latter is basically the probability of $B$ if $A$ happens. But if $A$ and $B$ are independent, $A$ provides no information about $B$, so the probability of $B$ doesn't change if $A$ happens. 
